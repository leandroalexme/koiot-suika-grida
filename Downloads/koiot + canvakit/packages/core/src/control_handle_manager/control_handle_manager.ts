...
export class ControlHandleManager {
  ...
  draw(rect: ITransformRect | null) {
    ...
    // 🔍 DEBUG: Log do tipo de renderer sendo usado
    if (renderer) {
      console.log('🚀 ControlHandles: Usando CanvasKit renderer');
    } else {
      console.log('⚠️ ControlHandles: Fallback Canvas2D');
    }
    
    handles.forEach((handle) => {
      const graphics = handle.graphics;
      if (graphics.type === GraphicsType.Path) {
        // TODO: Path handles - implementação futura
      } else {
      ...
      ctx.save();
      // 🚀 PASSA O RENDERER UNIFICADO para as Graphics
      graphics.draw({ ctx });
      ctx.restore();
    });
  }
