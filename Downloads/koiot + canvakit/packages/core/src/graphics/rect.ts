  ...
  private _realDraw(
          ...
          case PaintType.Image: {
            if (imgManager) {
              ...
              if (typeof (ctx as any).drawImage === 'function') {
                // Usar CanvasKit otimizado via renderer abstrato
                ctx.save();
                ctx.transform(...transform);
                this.fillImageCanvasKit(ctx, paint, imgManager, smooth, cornerRadius);
                ctx.restore();
    ...
    if (opacity !== 1) {
      ...
      if (canvas2DCtx) {
        ...
        console.warn('Rect drawing requer CanvasRenderingContext2D - operação não suportada');
      }
    }
