...
// Removido: imports não usados - star agora usa CanvasKit puro
...
export class SuikaStar extends SuikaGraphics<StarAttrs> {
  ...
  private _realDraw(
    ...
    const draw = (layerCtx: CanvasRenderingContext2D | import('../renderer').IRenderer) => {
      ...
      for (const paint of fill ?? []) {
        ...
        switch (paint.type) {
          ...
          case PaintType.Image: {
            if (imgManager) {
              // 🚀 MIGRAÇÃO CANVASKIT: Usar versão otimizada
              if (typeof (ctx as any).drawImage === 'function') {
                ctx.save();
                ctx.transform(...transform);
                ctx.clip();
                this.fillImageCanvasKit(ctx, paint, imgManager, smooth);
                ctx.restore();
                return; // Skip layer drawing
              } else {
                layerCtx.clip();
                this.fillImage(layerCtx as CanvasRenderingContext2D, paint, imgManager, smooth);
              }
            } else {
    ...
    if (opacity !== 1) {
      // CanvasKit puro: usar globalAlpha
