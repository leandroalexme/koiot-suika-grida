    ...
    for (const paint of fill ?? []) {
      ...
      switch (paint.type) {
        ...
        case PaintType.Image: {
          if (imgManager) {
            // 🚀 MIGRAÇÃO CANVASKIT: Usar drawImage nativo quando possível
            if (ctx && typeof (ctx as any).drawImage === 'function') {
              this.fillImageCanvasKit(ctx, paint, imgManager, smooth);
            } else {
              // Fallback para Canvas2D quando necessário
              const canvas2DCtx = this.getCanvas2DContext(ctx);
              if (canvas2DCtx) {
                this.fillImage(canvas2DCtx, paint, imgManager, smooth);
              } else {
                console.warn('fillImage: nenhum contexto compatível disponível');
              }
            }
          } else {
