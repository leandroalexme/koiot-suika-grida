...
// Removido: getCanvas2DContext - agora usamos apenas CanvasKit
import {
  type GraphicsAttrs,
  type IAdvancedAttrs,
  type IGraphicsOpts,
...
export class SuikaGraphics<ATTRS extends GraphicsAttrs = GraphicsAttrs> {
  ...
  /**
   * Verifica se o renderer é válido
   */
  protected isValidRenderer(renderer: import('../../renderer').IRenderer): boolean {
    return renderer !== null && typeof renderer.save === 'function';
  }

  /**
   * Versão CanvasKit otimizada para fill image
   * Usa drawImage nativo do CanvasKit quando possível
   */
  protected fillImageCanvasKit(
    ctx: CanvasRenderingContext2D | import('../../renderer').IRenderer,
    paint: PaintImage,
    imgManager: ImgManager,
    _smooth = true,
    cornerRadius = 0,
    ...
    // 🚀 OTIMIZAÇÃO CANVASKIT: Scale calculation otimizada
    const scale = calcCoverScale(img.width, img.height, width, height);
    const sx = img.width / 2 - width / scale / 2;
    const sy = img.height / 2 - height / scale / 2;
    ...
    if (cornerRadius && cornerRadius > 0) {
      ...
      // Usar roundRect nativo se disponível (CanvasKit ou Canvas2D moderno)
      if (typeof (ctx as any).roundRect === 'function') {
        ...
        drawRoundRectPath(ctx as any, 0, 0, width, height, cornerRadius);
        ctx.clip();
      }
    }

    // ✅ Usar drawImage com todas as otimizações
    ctx.drawImage(
      img,
      sx,
      sy,
