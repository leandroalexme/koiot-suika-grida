import { isWindows } from '../../common/src/common';
...
export class KeyBindingManager {
  ...
  private handleAction = (e: KeyboardEvent) => {
    ...
    let isMatch = false;
    const ctx: IWhenCtx = {
      isToolDragging: this.editor.toolManager?.isDragging() || false,
    };

    for (const keyBinding of Array.from(this.keyBindingMap.values())) {
      // match when
  ...
  registerWithHighPrior(keybinding: IKeyBinding) {
    ...
    for (const [key, val] of Array.from(this.keyBindingMap)) {
      map.set(key, val);
    }
